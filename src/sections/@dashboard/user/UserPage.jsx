...
// ----------------------------------------------------------------------
const getUserPhotoUrl = (photoUrl, userName) => {
...
const UserPage = () => {
  ...
  const addUser = (formData) => {
    axios.post(apiUrl(routes.USER, methods.POST), formData, {
  ...
  const updateUser = (formData) => {
    axios.put(apiUrl(routes.USER, methods.PUT, selectedUserId), formData, {
      headers: { 'Content-Type': 'multipart/form-data' }
    })
      .then((response) => {
        console.log(response.data);
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {users.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((user) => {
                        const photoSrc = getUserPhotoUrl(user.photoUrl, user.name);

                        return (
                          <TableRow hover key={user._id} tabIndex={-1}>
                            <TableCell align="left">
                              <Avatar alt={user.name} src={photoSrc} />
                            </TableCell>
